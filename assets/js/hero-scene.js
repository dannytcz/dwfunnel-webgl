...
/** mesh3d = real geometry (default) | portrait = 2.5D still | glb = external model */

const HERO_MODE = "mesh3d";



const GLB_URL = "assets/models/cyborg-bust.glb";
...
const bloom = new UnrealBloomPass(new THREE.Vector2(1, 1), 0.75, 0.45, 0.18);
...
scene.add(new THREE.HemisphereLight(0x39ff14, 0x030508, 0.25));
...
const rim = new THREE.DirectionalLight(0x39ff14, 2.2);
...
const eyeLight = new THREE.PointLight(0x39ff14, 4, 8);

eyeLight.position.set(0, 0.4, 1.5);
...
async function tryLoadGlb() {
  ...
  try {
    ...
    const model = await loadCyborgGlb(GLB_URL);
...
function updateLook() {
  ...
  /* whole bust leans — reads as volume, not flat card */

  rig.rotation.y = THREE.MathUtils.lerp(rig.rotation.y, mouse.tx * 0.12 * scrollDamp, 0.05);

  rig.rotation.x = THREE.MathUtils.lerp(rig.rotation.x, -mouse.ty * 0.06 * scrollDamp, 0.05);
...
function applyScrollLayout() {
  ...
  bloom.strength = THREE.MathUtils.lerp(0.75, 0.45, s);
...
function animate() {
  ...
  if (state.phase === "intro") {
    ...
    camera.position.z = THREE.MathUtils.lerp(5.2, 4.6, Math.min(elapsed / (INTRO_MS / 1000), 1));
    ...
    camera.position.x = THREE.MathUtils.lerp(camera.position.x, mouse.x * 0.35 * camDamp, 0.06);

    camera.position.y = THREE.MathUtils.lerp(camera.position.y, 0.08 + mouse.y * 0.22 * camDamp, 0.06);

    camera.position.z = 4.85;

    camera.lookAt(cyborg.position.x * 0.35 + mouse.x * 0.15, cyborg.position.y * 0.35, 0);
